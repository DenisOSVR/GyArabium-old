...
uint256 hashGenesisBlock("0x5ef01ee9a75076ee76eb374f83bd875263a6d2b588b946c7b0f26fe09b022307");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gyarabium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gyarabium Signed Message:\n";
...
{
    // Gyarabium: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 0.2 * COIN;

    // Subsidy is cut in half every 17500 blocks, which will occur approximately every 1 month
    nSubsidy >>= (nHeight / 17500); // Gyarabium: 17.5k blocks in ~1 month

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // Gyarabium: 1 day
static const int64 nTargetSpacing = 1 * 60; // Gyarabium: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gyarabium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gyarabium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb9215e849435ab607913e4e2da8c162524870b121abe32772da74001c31aad00");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Prague Morning 26/Jan/2022 Prague Will Have a New Metro Station: Depo Zlicin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0.5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0418eab1b7e748a6859dedd31d80d171907fc0451e0b4b07399f90881e589302c0549e9202b78c1380dceac01af8523e6df717d5705c26aec316ddee07e24d4e53") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1643228837;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086754620;

        if (fTestNet)
        {
            block.nTime    = 1643228800;
            block.nNonce   = 386467346;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x32338f2b5c4fee2240d2f401364a809133d6fd3d6c9f031ac835b7595f02def0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc6, 0xb9, 0xdb }; // Gyarabium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GyarabiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GyarabiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GyarabiumMiner(CWallet *pwallet)
{
    printf("GyarabiumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gyarabium-miner");
    ...
    try { loop {
        ...
        printf("Running GyarabiumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GyarabiumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GyarabiumMiner, pwallet));
}
